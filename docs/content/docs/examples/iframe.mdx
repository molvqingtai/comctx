...
<Steps>
  ...
  <Step>
    ...
    The same adapter works on both sides because the caller passes the window
    endpoint explicitly. The iframe page passes `window.parent`; the host page
    passes `window`.
  ...
  <Step>
    ...
    3. the host `WindowAdapter` sends the message through `window.postMessage`
    ...
    - the host page injects the proxy through `WindowAdapter`
