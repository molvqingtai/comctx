...
- `true` and `'message'` log all debug output.
- `'event'` logs only effective Comctx events.

`comctx:message` entries show messages observed by the adapter. They are useful when checking adapter wiring, but they can be noisy because one message can be seen by multiple listeners.

`comctx:event` entries show messages accepted by Comctx and routed into send or receive handling. Use this level when you only want to see the effective cross-context flow.
